...
class WeightedJointMOGP(gpf.models.GPModel, InternalDataTrainingLossMixin):
    def __init__(
        ...
        self.source_contribution = source_contribution
    ...
    def log_marginal_likelihood(self, *args, **kwargs) -> tf.Tensor:
        ...
        logdet_s = 2.0 * tf.reduce_sum(tf.math.log(tf.linalg.diag_part(Laa)))

        delta = By - mu_t
        alpha_t = tf.linalg.triangular_solve(L_t, delta)

        delta_s = Ay
        alpha_s = tf.linalg.triangular_solve(Laa, delta_s)
        # Math
        ...
        n_source = tf.cast(
            tf.shape(Ax)[0], Ax.dtype
        )  # We compute for the target (given source)
        quad_part_t = tf.matmul(alpha_t, alpha_t, transpose_a=True)
        quad_part_s = tf.matmul(alpha_s, alpha_s, transpose_a=True)

        lml_part_1 = -0.5 * (
            logdet_t
            + quad_part_t
            + n_target * tf.cast(tf.math.log(2 * np.pi), Ax.dtype)
        )
        lml_part_2 = -0.5 * (
            logdet_s
            + quad_part_s
            + n_source * tf.cast(tf.math.log(2 * np.pi), Ax.dtype)
        )
        return tf.squeeze(lml_part_1 + self.source_contribution * lml_part_2)
    ...
    def logdet(self):
        ...
        return -0.5 * logdet_t + 0.5 * logdet_s
...
class SparseCMOGP(gpf.models.GPModel, InternalDataTrainingLossMixin):
    ...
    def conditional_likelihood(
        ...
        W = Kmm + Kma @ tf.linalg.diag(1.0 / D_s) @ Kam  # [3]
    ...
    def predict_f(self, Xnew, full_cov: bool = False, full_output_cov: bool = False):
        ...
        # Construct Kmn
        Knm = (
            self.kernel(Xs, inducing_variable)
            @ tf.linalg.inv(Kmm)
    ...
    def expensive_predict_f(
        ...
        if self.exact_target:
            K_fitc = tf.concat(
                (tf.concat((Qaa + Lambda, Qba), 0), tf.concat((Qab, Kbb), 0)), 1
            )
        else:
            K_fitc = tf.concat(
                (
                    tf.concat((Qaa + Lambda, Qba), 0),
                    tf.concat((Qab, Qbb + Lambda_t), 0),
                ),
                1,
            )
        noise_ab = tf.concat(
