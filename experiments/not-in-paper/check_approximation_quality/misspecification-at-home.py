...
for i, prop in enumerate(np.arange(0.1, 1, 0.1)):
    ...
    for j in range(repetitions):
        ...
        kern = k * coreg 
        m = SparseCMOGP((X, y), conditioning_index=1, exact_target=False, jitter=0, inducing_variable=LMCInducingPointsBase(ivs), kernel=kern, likelihood=TransferLikelihood(source=gpf.likelihoods.Gaussian(), target=gpf.likelihoods.Gaussian()))
        ...
        print("SVGP:", svgp_mse)
        np.savez("results_at_home_matern32_misspecification", svgp=results_svgp, cmogp=results_cmogp)