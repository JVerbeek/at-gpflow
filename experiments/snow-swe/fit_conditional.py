...
for index in range(output_dim):
    ...
    if index == condition_index:
        fmean_test, fvar_test = model1.predict_f(np.hstack((Xtst, index * np.ones((len(Xtst), 1)))))
    ...
    plt.savefig(Path(__file__).parent / "figures" / f"fit_conditional_{HOLDOUT}_output{index}.png", dpi=120)
plt.show()

ours_mse = mean_squared_error(ytest, fmean_test)
print("held-out readings:", ytest[:, 0].round(1))
print("predicted:        ", fmean_test.numpy()[:, 0].round(1))
print("rmse:", np.sqrt(ours_mse).round(2), "in")
